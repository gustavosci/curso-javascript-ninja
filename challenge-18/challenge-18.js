...
(function(){
	...
	*/
	...
	function cleanCPF(arrCPF){
		for(var i = 0; i < arrCPF.length; i++){
			arrCPF[i] = arrCPF[i].replace(/[^\d]/g, "");
			console.log( arrCPF[i] );
		}
		return arrCPF;
	}
	...
	*/
	function formatCPF(arrCPF){
		for(var i = 0; i < arrCPF.length; i++){
			arrCPF[i] = arrCPF[i].replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, function includeMask(capturaTotal, d1, d2, d3, d4){
				return d1 + "." + d2 + "." + d3 + "-" + d4; 
			});
			console.log( arrCPF[i] );
		}
	...
	*/
	...
	console.log( frase.match(/[j][u].[h][o]/gi) );
	...
	*/
	html = "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
	console.log( '\nMatch com tags HTML vazias (abertura e fechamento da tag):' );
	console.log( html.match(/<\w+><\/\w+>/gi) );
	...
	*/
	...
	console.log( html.replace(/<(\w+)>([^<]+)<\/(\w+)>/gi, function (capturaTotal, tag, valor, closeTag){
		return "<" + tag + ">" + "O texto dentro da tag " + tag + " é " + valor + "</" + closeTag + ">";
	}) );
