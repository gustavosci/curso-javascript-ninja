(function (){
	...
	*/
	function isOperatorValid(ope){
		return (ope === "+" || ope === "-" || ope === "*" || ope === "/" || ope === "%") ? true : false;
	}
	...
	*/
	function calculator(ope){
		// Se o operador não é válido
		if(! isOperatorValid(ope)){
			return false;
		}
		return function secondCalculador(x, y){
			// Se algum parâmetro não é número
			if(typeof x !== "number" || typeof y !== "number"){
				return false;
			}
			return operation[ope](x, y);
		}
	}
	...
	*/
	function showOperationMessage(ope, x, y){
		console.log("A operação " + x + " " + ope + " " + y + " = ");
	}
	...
	*/
	function showErrorMessage(opeErr){
		console.log("Operação " + opeErr + " não permitida!");
	}
	...
	*/
	if(sum){
		number1 = 16;
		number2 = 10;
		showOperationMessage("+", number1, number2);
		sum(number1, number2);
	}else{
